...
<template>
    <Teleport to="body">
        <Transition name="modal">
            <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3 backdrop-blur-sm sm:p-4">
                <div class="modal-content w-full max-w-3xl rounded-4xl bg-white p-4 dark:bg-gray-800 sm:p-6">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <input v-model="form.counterparty" type="text" placeholder="输入交易方" class="input-base w-full" />
                        </div>

                        <div class="space-y-2">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">交易类型:</label>
                            <input v-model="form.category_name" type="text" placeholder="输入类型" class="input-base w-full" />
                        </div>

                        <div class="space-y-2">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">金额:</label>
                            <input v-model="form.amount" type="number" inputmode="decimal" placeholder="输入金额" class="input-base w-full"
                    ...
                    <div class="mt-8 flex items-center justify-end gap-3">
                        ...
                        <button class="button-base bg-green-600 text-white hover:bg-green-700 disabled:opacity-60" :disabled="!canSubmit"
                            @click="submit">
                            {{ submitting ? "提交中..." : "确认保存" }}
                        </button>
                    </div>
